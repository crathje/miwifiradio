...
if($pid[0]=='')
{
	...
	$ffmpeg_codec = '-c:a libfdk_aac'; // "original"
	//$ffmpeg_codec = '-strict -2 -c:a aac'; // working for ubuntu 16.04

	$ffmpegcmd = '/usr/local/bin/ffmpeg -headers \'User-Agent: "Dalvik/1.6.0 (Linux; U; Android 4.2.2; C2004 Build/15.2.A.2.5)"\' -i "'.$radioRow['streamurl'].'"  '.$ffmpeg_codec.' -b:a 64k -f ssegment -segment_list '.escapeshellarg($global_path . 'uploads/playing/' . $xid . '/pl.m3u8').' -segment_list_flags +live -segment_time 7 -segment_list_size 3 -segment_wrap 5 -segment_list_entry_prefix '.escapeshellarg('http://'.$domain.'/uploads/playing/' . $xid . '/').' ' . escapeshellarg($global_path . 'uploads/playing/' . $xid . '/64%03d.aac') . '  > /dev/null 2>&1 < /dev/null &';
	//echo $ffmpegcmd;
	exec($ffmpegcmd);
    sleep(7);
